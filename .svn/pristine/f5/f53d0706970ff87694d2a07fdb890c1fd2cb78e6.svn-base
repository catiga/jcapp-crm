package com.jeancoder.crm.entry.h5.user
...
try {
	...
	String mc_pwd = request.getParameter("mc_pwd");
...
//		return result.setData(AvailabilityStatus.notAvailable("密码为空"));
	...
	if (!"0".equals(mcr.outer_type)) {
		return result.setData(AvailabilityStatus.notAvailable("外部会员卡不支持开卡"));
	}
