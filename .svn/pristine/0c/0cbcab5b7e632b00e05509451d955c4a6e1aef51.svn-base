...
class AccountProjectMcService {
	 ...
	 * @return
	 */
	public AccountProjectMC get_normal_mc_by_id(def pid, def id) {
		String hql = "select * from AccountProjectMC m where flag!=? and id=? and status=?";
		AccountProjectMC item = jcTemplate.get(AccountProjectMC.class,hql, -1, id, AccountProjectMCStatus.NORMAL);
		if (item == null) {
			return null;
		}
		return item;
	...
	public List<AccountProjectMC> get_by_apid_or_mobile(BigInteger basic_id,String mobile){
		def p = [];
		String sql ="select * from AccountProjectMC where flag!=? and status=? and ( ";
		p.add(-1);
		p.add(AccountProjectMCStatus.NORMAL);
		String s = "";
		if (basic_id != null) {
			s += " basic_id=? " 
			p.add(basic_id);
		}
		if (!StringUtil.isEmpty(s) && !StringUtil.isEmpty(mobile))  {
			s += " or "
		}
		...
		sql += s;
		sql += " )";
		List<AccountProjectMC> result = jcTemplate.find(AccountProjectMC.class, sql, p.toArray());
		return result;
	}
	 ...
	 * @return
	 */
	public def get_by_num(String mc_num,BigInteger pid){
		String sql="select * from AccountProjectMC where mc_num=? and flag!=? and pid=?  and status=? ";
		return jcTemplate.find(AccountProjectMC.class, sql, mc_num,-1,pid,AccountProjectMCStatus.NORMAL);
	}
	 ...
	 * @return
	 */
	public AccountProjectMC init_local_mc(def  pid, def appid, String mc_num, String mc_pwd) {
		//首先校验该会员卡号在该项目下是否已经绑定
		String sql = "select * from AccountProjectMC a where flag!=? and pid=? and mc_num=?";
		List<AccountProjectMC> result = jcTemplate.find(AccountProjectMC, sql, -1,pid,mc_num)
		if(result!=null&&!result.isEmpty()) {
			throw new ObjectExistExceptio("会员卡卡号重复");
		}
		AccountProjectMC  entity = new AccountProjectMC();
		entity.pid = new BigInteger(pid.toString());
		entity.basic_id = new BigInteger(appid) ;
		entity.mc_num = mc_num;
		entity.mc_pwd = mc_pwd;
		entity.a_time = new Date();
		entity.status = AccountProjectMCStatus.NORMAL;
		def id  = JcTemplate.INSTANCE().save(entity);
		entity.id = id;
		return entity;
	}
}
