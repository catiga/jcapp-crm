...
try {
	...
	String outer_uri = request.getParameter("outer_uri");
	String outer_pc_num = request.getParameter("outer_pc_num");
	String outer_pc_key = request.getParameter("outer_pc_key");
	
	Long project_id = RemoteUtil.getProj().id;
	if (StringUtil.isEmpty(mcr_title) || StringUtil.isEmpty(mcr_info) || StringUtil.isEmpty(mcr_outer_type) || StringUtil.isEmpty(mcr_supp_jf)) {
	...
	mcr.id = new BigInteger(bid.toString());
	mcr.title = mcr_title;
	mcr.info = mcr_info;
	...
	mcr.outer_uri = StringUtil.trim(outer_uri);
	mcr.outer_pc_num = StringUtil.trim(outer_pc_num);
	mcr.outer_pc_key = StringUtil.trim(outer_pc_key);
	String resultStr = MemberCardRuleService.INSTANSE.updateItem(mcr);
