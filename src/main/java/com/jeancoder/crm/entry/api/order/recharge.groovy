...
try {
	...
	if ("refund".equals(op)) {
		...
		if (!OrderConstants._order_status_payed_pub_failed_.equals(order.order_status)) {
			// 充值成功不支持退款
			return rules.setData(NotifyObj.build("-1", "充值成功不支持退款", "", "",null ));
		}
		// 充值失败 分两种情况 一种是有充值记录
	...
	if (detail != null && !detail.isEmpty()) {
		return NotifyObj.build("-1","订单重复", "", "",null )
	}
	/***************************开始充值*******************************************/
	OrderRechargeMc order = OrderRechargeService.INSTANSE.getByNum(on, pid);
	if (order == null) {
	...
	try {
		...
		if (mch != null && !"0".equals(mch.getMcRule().outer_type)) {
			...
			Logger.error("recharge_money " + recharge_amount);
