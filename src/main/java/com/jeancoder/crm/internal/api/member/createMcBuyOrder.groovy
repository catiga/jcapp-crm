...
String mc_name = JC.internal.param("mc_name");//会员姓名
String mc_mobile = JC.internal.param("mc_mobile");//会员手机号
String mc_code = JC.internal.param('mc_code');
String id_card = JC.internal.param("id_card");// 身份证号码
String pwd = JC.internal.param("pwd");// 密码
String pre_id = JC.internal.param("pre_id");//  getMcNum 接口 会返回一个对象， 对象里面有id

def sid = JC.internal.param('sid');
def sname = JC.internal.param('sname');
def log_id = JC.internal.param('log_id');
def pid = JC.internal.param('pid');
def auth_token = JC.internal.param('auth_token');
if(auth_token) {
	auth_token = JackSonBeanMapper.fromJson(auth_token, AuthToken);
} else {
...
try {
	...
	OrderMc orderMc = OrderMcService.INSTANSE.admin_create_order(card_code, mch,pid,new BigInteger(sid),sname,new BigInteger(pre_id) , mc_name,mc_mobile,id_card,pwd, auth_token);
