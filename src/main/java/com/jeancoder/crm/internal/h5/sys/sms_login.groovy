...
if(third==null) {
	return SimpleAjax.notAvailable('账户初始化失败，请重试'+apid);
}
...
AccountSession session = session_service.login_session(user.mobile, user.password, third, validate_period, "0", third.pid);

try {
	// 所有会员卡的pid 均为1
	AccountProjectMcService.INSTANSE.updateByMobile(user.mobile,session.ap_id,third.pid);
}catch(any) {
}
